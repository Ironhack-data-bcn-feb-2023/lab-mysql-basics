...
SELECT district_id, COUNT(client_id) AS count FROM client
...
SELECT type, COUNT(card_id) AS count FROM card
	GROUP BY type
    ORDER BY count DESC;
...
-- Query 16: n the loan table, retrieve the number of loans issued for each day, before (excl) 930907, ordered by date in descending order.
SELECT date, COUNT(loan_id) AS count_loan FROM loan
	GROUP BY date
	HAVING date < 930907
    ORDER BY date DESC;
...
SELECT date, duration, COUNT(loan_id) AS count_loan FROM loan
